...
`scripts/update_literature.py` 每次从 Europe PMC 检索 `2018-01-01` 至运行当天的新候选文献，检索终点会随日期自动推进。脚本不会猜测影响因子，也不会自动发布未审阅内容；新候选进入 `data/review_queue.json`，审核后再合并进 `data/papers.json`。
